...
const CreateTodo = ({ todoId }) => {
  ...
  const createTodo = async () => {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/crud`, {
        ...
        body: JSON.stringify({ ...todos, userId: user._id }),
      });
  ...
  return (
    (user ?
      ...
      </div> : <div className="max-w-2xl mx-auto p-4 sm:p-8 bg-gradient-to-br from-blue-50 to-purple-50 rounded-xl sm:rounded-2xl shadow-[0_8px_30px_-12px_rgba(0,0,0,0.1)] border border-blue-100/80 animate-pulse">
        <div className="mb-6 sm:mb-8 text-center">
          <div className="h-6 sm:h-8 w-48 mx-auto bg-blue-100/80 rounded-full mb-2" />
          <div className="h-4 w-64 mx-auto bg-blue-100/60 rounded-full" />
        </div>

        <div className="space-y-4 sm:space-y-6">
          <div className="space-y-2">
            <div className="h-4 w-20 bg-blue-100/70 rounded-full" />
            <div className="h-12 bg-white border border-blue-100 rounded-xl" />
          </div>

          <div className="space-y-2">
            <div className="h-4 w-24 bg-blue-100/70 rounded-full" />
            <div className="h-40 bg-white border border-blue-100 rounded-xl" />
          </div>

          <div className="flex flex-col sm:flex-row items-center gap-3 sm:justify-end pt-6">
            <div className="h-10 w-28 bg-blue-100/70 rounded-full" />
            <div className="h-10 w-32 bg-blue-100/90 rounded-full" />
          </div>
        </div>
      </div>)
  );
